...
1. **Fetch** — Pulls the next issue from Linear, Trello, GitLab Issues, or Jira matching the configured label, team, and project. Issues are sorted by priority. Blocked issues are skipped.
...
| `lisa run --source NAME` | Override issue source (linear, trello, gitlab-issues, jira) |
...
| Field | Linear | Trello | GitLab Issues | Jira |
|-------|--------|--------|---------------|------|
| `team` | Team name | Board name | Project path (`namespace/project`) or numeric ID | Project key (e.g. `ENG`) |
...
| `in_progress` | In-progress status | In-progress column | Label to apply on activate | In-progress status name |
| `done` | Destination status after PR | Destination column after PR | Closes the issue | Destination status after PR |
