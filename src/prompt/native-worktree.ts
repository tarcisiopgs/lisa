...
export interface NativeWorktreePromptContext {
	issue: Issue;
...
export function buildNativeWorktreeInstructions(ctx: NativeWorktreePromptContext): string {
	const {
		issue,
		testRunner,
		pm,
	...
	const branchRenameInstruction = `   rename it: \`git branch -m <current-name> feat/${issue.id.toLowerCase()}-short-english-slug\``;
