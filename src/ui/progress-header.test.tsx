...
function stripAnsiAndExtractContent(output: string | undefined | null): string {
  ...
  const ansiStripRegex = new RegExp('\\u001b\\[[0-9;]*m', 'g');
  ...
  if (lines.length >= 2) {
    ...
    if (contentLine !== undefined) {
        return contentLine.replace(/^[┌└ ]*│\s*/, '').replace(/\s*│[┐┘ ]*$/, '').trim();
    }
  }
...
describe("ProgressHeader", () => {
	...
	it("renders 100% green when workComplete is true", () => {
		...
		expect(stripAnsiAndExtractContent(lastFrame())).toContain("████████████████████ 5/5 (100%)");
		expect(stripAnsiAndExtractContent(lastFrame())).not.toContain("░"); // No remaining bars within the content line
	});
	...
	it("renders without running cards when running is 0", () => {
		...
		expect(stripAnsiAndExtractContent(lastFrame())).not.toContain("(0 running)"); // Still no (0 running)
	});
