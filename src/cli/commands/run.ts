...
export async function executeRun(args: Record<string, unknown>): Promise<void> {
	...
	const merged = mergeWithFlags(config, {
		...
		bell: args.bell as boolean | undefined,
	});
	...
	const concurrency = Math.max(1, Number.parseInt(args.concurrency as string, 10) || 1);
	...
	await runLoop(merged, {
		once: (args.once as boolean) || !!(args.issue as string),
...
export const run = defineCommand({
	meta: {
		name: "run",
		description: "Alias for `lisa` — fetch issues, run AI agents, and deliver pull requests",
	},
	args: runArgs,
	async run({ args }) {
